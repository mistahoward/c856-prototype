...
const accommodations = [
	{
		...
		description:
			'A luxurious resort offering overwater bungalows with stunning views of the lagoon.',
		lat: -16.499,
		lng: -151.737,
		image: 'lagoon_paradise.jpg',
		expensive_price: 1500,
		expensive_info:
			'Includes a private villa, gourmet dining, spa treatments, and a personal tour guide.',
		moderate_price: 950,
		moderate_info:
			'Features an overwater bungalow, access to all resort amenities, and a complimentary dinner.',
		cheapest_price: 600,
		cheapest_info:
			'Offers a cozy beachside room with beautiful views and access to recreational activities.',
	},
	{
		id: 2,
		title: 'Blue Lagoon Inn',
		description:
			'A charming inn located near Matira Beach, known for its friendly service and local cuisine.',
		lat: -16.5415,
		lng: -151.7381,
		image: 'blue_lagoon.jpg',
		expensive_price: 1200,
		expensive_info:
			'A spacious suite with ocean views, private dining, and an exclusive island tour.',
		moderate_price: 750,
		moderate_info:
			'Comfortable accommodation with beach access, breakfast included, and a snorkeling trip.',
		cheapest_price: 500,
		cheapest_info:
			'A standard room with essential amenities, perfect for budget-conscious travelers.',
	},
	{
		id: 3,
		title: 'Sunset Beach Bungalows',
		description:
			'Beachfront bungalows offering a unique blend of traditional design and modern comfort.',
		lat: -16.5055,
		lng: -151.753,
		image: 'sunset_beach.jpg',
		expensive_price: 1300,
		expensive_info:
			'An exclusive bungalow with a private beach area, all meals included, and a sunset cruise.',
		moderate_price: 800,
		moderate_info:
			'A deluxe bungalow with sea views, complimentary breakfast, and a guided lagoon tour.',
		cheapest_price: 550,
		cheapest_info:
			'A comfortable bungalow with easy beach access and free kayak rentals.',
	},
];
...
try {
	...
	for (const item of accommodations) {
		const query = `INSERT INTO accommodations (id, title, description, lat, lng, image, expensive_price, expensive_info, moderate_price, moderate_info, cheapest_price, cheapest_info) VALUES (${
			item.id
		}, '${escapeSql(item.title)}', '${escapeSql(item.description)}', ${
			item.lat
		}, ${item.lng}, '${escapeSql(item.image)}', ${
			item.expensive_price
		}, '${escapeSql(item.expensive_info)}', ${
			item.moderate_price
		}, '${escapeSql(item.moderate_info)}', ${
			item.cheapest_price
	...
	for (const item of destinations) {
		const query = `INSERT INTO destinations (id, title, description, detailed_description, lat, lng, image) VALUES (${
			item.id
		}, '${escapeSql(item.title)}', '${escapeSql(
			item.description
		)}', '${escapeSql(item.detailed_description)}', ${item.lat}, ${
			item.lng
	...
	for (const item of reviews) {
		const query = `INSERT INTO reviews (name, review, rating, image, date) VALUES ('${escapeSql(
			item.name
		)}', '${escapeSql(item.review)}', ${item.rating}, '${escapeSql(
			item.image
		)}', ${item.date});`;
		fs.writeFileSync(tempSqlFile, query);
		run(`npx wrangler d1 execute ${dbName} --file=${tempSqlFile}`);
