...
for (const accommodation of accommodations) {
	run(`npx wrangler d1 insert ${dbName} --data='${JSON.stringify(accommodation)}'`);
}

for (const destination of destinations) {
	run(`npx wrangler d1 insert ${dbName} --data='${JSON.stringify(destination)}'`);
}

for (const review of reviews) {
	run(`npx wrangler d1 insert ${dbName} --data='${JSON.stringify(review)}'`);
}
