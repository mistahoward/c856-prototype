...
const AccommodationPage: FC<PageProps> = ({ location }) => {
	...
	const {
		...
		error,
		data: apolloData,
	} = useQuery(GET_ACCOMMODATION_BY_ID, {
		...
		return (
			<Layout>
				<p className="text-center mt-5">
					Sorry, this accommodation could not be found.
				</p>
			</Layout>
		);
	}
		...
		<Container fluid id="root">
			<Layout>
				<Row className="align-items-center mb-4">
					<Col xs={12} md={8}>
						<h1 className="display-4">{accommodation.title}</h1>
						<p className="lead">{accommodation.description}</p>
					</Col>
